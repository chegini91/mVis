...
/**
 * ****************************************************************************
 * Compilation: javac -cp .:jama.jar PolynomialRegression.java Execution: java
 * -cp .:jama.jar PolynomialRegression Dependencies: jama.jar
 *
 * % java -cp .:jama.jar PolynomialRegression 0.01 n^3 + -1.64 n^2 + 168.92 n +
 * -2113.73 (R^2 = 0.997)
 *
 *****************************************************************************
 */
 ...
 * &beta;<sub>2</sub> <em>x</em><sup>2</sup> + ... + &beta;<sub><em>d</em></sub>
 * <em>x</em><sup><em>d</em></sup>
 * (where <em>y</em> is the response variable, <em>x</em> is the predictor
 * variable, and the &beta;<sub><em>i</em></sub> are the regression
 * coefficients) that minimizes the sum of squared residuals of the multiple
 * regression model. It also computes associated the coefficient of
...
public class PolynomialRegression implements Comparable<PolynomialRegression> {
     ...
     * {@code (y[i], x[i])}.Uses n as the name of the predictor variable.
     *
     ...
     * equal
     */
    public PolynomialRegression(ArrayList<Point2D> selectedPoints, int degree) {
        this(selectedPoints, degree, "n");
    }
     ...
     * {@code (y[i], x[i])}.
     *
     * @param selectedPoints values of predictor
     * @param degree the degree of the polynomial to fit
     * @param variableName the name of the predictor variable
     * @throws IllegalArgumentException if the lengths of the two arrays are not
     * equal
     */
    public PolynomialRegression(ArrayList<Point2D> selectedPoints, int degree, String variableName) {

        this.degree = degree;
        this.variableName = variableName;

        double[] x = new double[selectedPoints.size()];
        double[] y = new double[selectedPoints.size()];
        for (int i = 0; i < selectedPoints.size(); i++) {
            x[i] = selectedPoints.get(i).getX();
            y[i] = selectedPoints.get(i).getY();
        }
     ...
     * including the best-fit polynomial and the coefficient of determination
     * <em>R</em><sup>2</sup>
     */
